...
export interface ServiceStartOptions {
  ...
  /** Reserved for the supervisor service mode supported by newer CLIs. */
  webUi?: boolean;
 ...
 */
function requireAdapter(cmdName: string, profile: string, runArgs?: string[]): ServiceAdapter {
  const adapter = getServiceAdapter(profile, runArgs);
  if (!adapter) {
    console.error(
      `${cmdName}: 当前系统不支持后台运行。`,
    );
...
export async function runServiceStop(opts: ServiceProfileOptions = {}): Promise<void> {
  ...
  const adapter = requireAdapter('stop', serviceId, undefined);
  ...
  const runtime = profile ? await maybeResolveProfileRuntime(profile) : undefined;
...
export async function runServiceRestart(opts: ServiceProfileOptions = {}): Promise<void> {
  const { serviceId, profile, webUi } = await resolveServiceTarget(opts);
  const adapter = requireAdapter('restart', serviceId, webUi ? ['run', '--web-ui'] : classicRunArgs(serviceId));
  if (!adapter.fileExists()) {
    console.error(webUi ? 'supervisor 还没在后台运行过。请先运行 `start --web-ui` 启动。' : 'bot 还没在后台运行过。请先运行 `start` 启动。');
    process.exit(1);
  }
...
export async function runServiceStatus(opts: ServiceProfileOptions = {}): Promise<void> {
  const { serviceId, profile, webUi } = await resolveServiceTarget(opts);
  const adapter = requireAdapter('status', serviceId, undefined);
  if (!adapter.fileExists()) {
    const label = webUi ? '控制面 supervisor' : 'bot';
    console.log(`${label} 当前没在后台运行(从未启动过)`);
    console.log(`  通过 ${webUi ? '`start --web-ui`' : '`start`'} 启动`);
    return;
  }
  if (!adapter.isRunning()) {
    const label = webUi ? '控制面 supervisor' : 'bot';
    console.log(`${label} 当前没在后台运行`);
    console.log(`  通过 ${webUi ? '`start --web-ui`' : '`start`'} 重新启动`);
    return;
  }
...
export async function runServiceUnregister(opts: ServiceProfileOptions = {}): Promise<void> {
  ...
  const adapter = requireAdapter('unregister', serviceId, undefined);
